...
export interface WaitlistCheckoutPaymentStatus {
  paymentId: string
  orderId: string
  status: string
  provider: string
...
export async function getWaitlistCheckoutPaymentStatus(paymentId: string | null): Promise<WaitlistCheckoutPaymentStatus | null> {
  ...
  if (error) {
    console.error("[waitlist-checkout] payment status:", error)
    return null
  }
