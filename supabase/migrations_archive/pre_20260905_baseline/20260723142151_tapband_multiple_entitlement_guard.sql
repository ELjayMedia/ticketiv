-- TICK-294 - make TapBand multi-ticket behavior explicit.
-- A TapBand with more than one currently admissible entitlement must not
-- silently consume one ticket. Return a distinct scanner state and audit row
-- until a fast ticket-selection UI is available.
