-- TICK-47: real organization creation for the organizer onboarding flow.
--
-- Until now onboarding was a client-only mock (it fabricated an org id and
-- wrote a demo session) so a real signed-in user never actually got an
-- organization. This adds a server-authoritative creation path: it inserts
-- the organization and the caller's owner membership in one transaction,
-- deriving a unique slug from the name.
...
begin
  ...
  -- Slugify: lowercase, non-alphanumerics to hyphens, collapse + trim.
