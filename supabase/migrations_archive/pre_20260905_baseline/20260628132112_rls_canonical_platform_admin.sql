...
create policy event_categories_select on public.event_categories for select to anon, authenticated using (is_active = true or app.is_platform_admin());
...
create policy webhooks_org_read on public.webhooks for select to public
...
create policy event_live_stats_org_read on public.event_live_stats for select to authenticated
  using (
    exists (select 1 from public.events e where e.id = event_live_stats.event_id
