...
--
-- Goals:
--   1. Hold the buyer's pending order for a finite window so abandoned carts
--      release inventory back to the quota.
--   2. Enforce per_user_limit across the buyer's existing orders, not just
--      the new request, and enforce ticket_type_channels limits/quotas.
--   3. Make order_adjustments contribute to orders.total_cents so a promo
--      code or fee actually changes what the buyer is charged.
--   4. Provide a SECURITY DEFINER entry point to validate and apply a promo
--      code against a pending order before payment starts, and record the
--      redemption only after the rule passes.
...
--    The original is preserved in git history; this replaces it in place to
...
begin
  ...
  -- Lock ticket_types to serialize concurrent buyers for the same event.
  perform 1
  ...
  loop
    ...
    -- Cross-order per_user_limit: count this buyer's tickets across pending
    -- (non-expired) and paid orders for this ticket_type.
    select count(*)::integer
      into v_existing_user_qty
    ...
    -- Channel availability: if ticket_type_channels has any rows for this
    -- ticket_type, the online channel must be one of them and within its
    -- per_order_limit / channel quota.
    if exists (select 1 from public.ticket_type_channels c where c.ticket_type_id = v_ticket_type.id) then
      select c.quota, c.per_order_limit
        into v_channel_row
    ...
    -- Overall quota across channels. Expired pending orders are excluded so
...
-- ----------------------------------------------------------------------------
-- 3. fn_apply_pricing_to_order now folds order_adjustments into the total.
--    Discounts are negative amount_cents, fees/taxes are positive. The
--    existing trigger reruns this whenever order_items change, and a new
--    trigger reruns it whenever order_adjustments change.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if not found then
    raise exception 'No active pricing plan for org %', v_org_id;
  end if;
  ...
  -- Clamp to zero — a promo code shouldn't make us owe the buyer money.
  ...
  -- Idempotent ledger rewrite for the planning entries. The payment-success
  -- path adds payment_net separately.
...
-- Trigger so order_adjustments changes refresh the order total too.
...
-- ----------------------------------------------------------------------------
-- 4. Promo code application. Returns { applied, adjustment_cents, label,
--    rule_id, reason } so the caller can surface feedback. Only writes
--    order_adjustments + price_rule_redemptions when the rule is valid.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if v_order.buyer_id <> p_user_id then
    return jsonb_build_object('applied', false, 'reason', 'order_not_owned');
  end if;
  if v_order.status <> 'pending' then
    return jsonb_build_object('applied', false, 'reason', 'order_not_pending');
  end if;

  -- Event id is derived from the first order_item's ticket_type.
  ...
  if v_event_id is null then
    return jsonb_build_object('applied', false, 'reason', 'order_has_no_items');
  end if;
  ...
  if not found then
    return jsonb_build_object('applied', false, 'reason', 'code_invalid');
  end if;
  ...
  -- Reject reapplying the same code to the same order.
  if exists (
