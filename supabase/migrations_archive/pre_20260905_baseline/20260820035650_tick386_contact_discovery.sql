alter table public.user_privacy_settings add column if not exists discover_by_phone boolean not null default false;
...
$$;
revoke all on function public.fn_update_my_social_privacy(text, boolean, boolean, boolean, boolean) from public, anon;
grant execute on function public.fn_update_my_social_privacy(text, boolean, boolean, boolean, boolean) to authenticated;
...
begin
  ...
  with inputs as (
    ...
    from inputs i
    join public.profiles p on i.phone_key is not null and public.fn_contact_phone_key(p.phone) = i.phone_key
...
comment on function public.fn_match_friend_contacts(text[]) is 'TICK-386: transiently matches user-selected phone contacts. Never returns or persists phone values.';;
