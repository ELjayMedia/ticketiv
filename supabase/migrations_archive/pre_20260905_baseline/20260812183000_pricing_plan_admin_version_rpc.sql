-- TICK-351: atomic, auditable pricing-plan version creation for the super-admin UI.
--
-- This function is deliberately service-role only. Browser/authenticated roles
-- cannot call it directly; the server action supplies the already-authenticated
-- super-admin actor id. It never mutates an existing plan's fee values. Instead,
-- it retires the current active row and inserts a new effective-dated version.
...
begin
  ...
  -- Serialize plan changes per organization so two admin saves cannot both
  -- retire/insert against the same active version concurrently.
