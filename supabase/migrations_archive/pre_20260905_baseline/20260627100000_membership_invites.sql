-- TICK-273 (S8): invite-based membership via token.
--
-- Org owners/admins generate a token-scoped invite for either an org role
-- (org_members) or an event door/scanner role (event_staff). On acceptance the
-- membership is bound to whoever is logged in (the token, not an email match), so
-- invited-email != login-email works. Tokens expire, are single-use, and can be
-- revoked. Mutations go through SECURITY DEFINER RPCs; the invitee never needs
-- direct write access to org_members/event_staff.
...
-- Org admins can view/list their org's invites. Mutations go through the RPCs.
...
-- Create an invite (org admin only).
...
-- Accept an invite — binds the membership to the CURRENT logged-in user.
...
-- Revoke an unaccepted invite (org admin only).
