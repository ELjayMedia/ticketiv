-- Fix auth_rls_initplan advisor warnings on refunds_insert and pos_shifts_select.
-- Wrap direct auth.uid() calls in scalar subqueries so the planner evaluates them
-- once per statement (InitPlan) instead of once per row. Semantics are unchanged.
