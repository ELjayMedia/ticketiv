-- TICK-41: provider-driven completion for resale & waitlist checkout.
--
-- The buyer-facing fn_complete_*_after_payment functions authorize via
-- auth.uid(), so a payment-provider webhook (which has no user session)
-- cannot call them. These service-role variants resolve the buyer from the
-- order instead, are idempotent (a second call after completion is a no-op
-- that returns the existing result), and are callable only by the service
-- role used by the verified webhook handler.
...
begin
  ...
  if v_payment.payload ->> 'kind' is distinct from 'resale_checkout' then
    raise exception 'payment is not a resale checkout' using errcode = 'P0001';
  end if;
  ...
  -- Idempotent: already sold means the webhook (or buyer) already finished.
  ...
  set status = 'sold', transfer_id = v_transfer_id, updated_at = now()
...
begin
  ...
  if v_payment.payload ->> 'kind' is distinct from 'waitlist_checkout' then
    raise exception 'payment is not a waitlist checkout' using errcode = 'P0001';
  end if;
