-- TICK-231: Add optional date range parameters to fn_org_finance_summary.
-- p_from / p_to default to NULL (no filter = all-time, same as before).
-- This is a DROP + RECREATE because PostgreSQL cannot add optional params
-- to an existing function in-place with the same signature.
...
begin
  ...
  if not (public.is_org_member(p_org_id, (select auth.uid())) or public.is_super_admin((select auth.uid()))) then
  ...
  -- Payout figures are not date-scoped (they reflect the running balance)
  select
