...
begin
  ...
  -- Serialize competing purchases for the same ticket types.
  perform 1
  ...
  for v_ticket_type in
    select tt.id, tt.event_id, tt.name, tt.price_cents, tt.currency, tt.quota, tt.per_user_limit, e.org_id, ci.quantity
    from public.ticket_types tt
...
revoke all on function public.fn_create_inventory_protected_order(uuid, uuid, text, jsonb, text) from public;
grant execute on function public.fn_create_inventory_protected_order(uuid, uuid, text, jsonb, text) to authenticated, service_role;

comment on function public.fn_create_inventory_protected_order(uuid, uuid, text, jsonb, text)
is 'Creates a pending order and pending order_items atomically. Locks selected ticket_types and prevents quota oversell by counting pending/paid reservations.';;
