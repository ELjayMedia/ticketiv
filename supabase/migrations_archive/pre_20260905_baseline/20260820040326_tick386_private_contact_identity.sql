create table if not exists public.user_private_profiles (
  ...
  constraint user_private_profiles_phone_check check (
    phone is null or regexp_replace(phone, '[^0-9+]', '', 'g') ~ '^\+?[0-9]{7,15}$'
  )
);
...
create policy user_private_profiles_select_self on public.user_private_profiles for select to authenticated using (user_id = (select auth.uid()));

insert into public.user_private_profiles (user_id, name, surname, phone)
select user_id, name, surname, phone from public.profiles
...
grant select (user_id, display_name, created_at, name, surname, locale, avatar_url) on table public.profiles to anon, authenticated;
update public.profiles set phone = null where phone is not null;
drop index if exists public.profiles_contact_phone_key_idx;
create index if not exists user_private_profiles_contact_phone_key_idx on public.user_private_profiles ((public.fn_contact_phone_key(phone))) where phone is not null and btrim(phone) <> '';

create or replace function public.fn_bootstrap_ticketiv_user(p_user_id uuid, p_email text default null, p_phone text default null, p_display_name text default null)
...
create or replace function public.fn_update_my_profile_unchecked(p_display_name text default null, p_name text default null, p_surname text default null, p_phone text default null)
...
create or replace function public.fn_complete_organizer_signup(p_first_name text, p_surname text, p_phone text, p_id_number text default null)
returns jsonb language plpgsql security definer set search_path to 'pg_catalog', 'app', 'public', 'private' as $$
declare
...
returns table(input_index integer, handle text, display_name text, avatar_url text, relationship_state text, can_request boolean)
