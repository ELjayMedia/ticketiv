-- Only genuinely external effects belong in the outbox. The notify* helpers
-- are plain inserts into public.notifications, so they are transactional and
-- move inside fn_complete_order_payment; ticket delivery (email/WhatsApp) is
-- the one effect that must happen after commit.

delete from public.payment_outbox where topic = 'payment_succeeded';

alter table public.payment_outbox drop constraint if exists payment_outbox_topic_check;
alter table public.payment_outbox add constraint payment_outbox_topic_check
  check (topic in ('ticket_delivery'));
...
begin
  ...
  -- In-app notifications are plain inserts into a durable queue, so they are
  -- transactional and belong here rather than in the outbox.
  if v_order.buyer_id is not null then
    ...
    insert into public.notifications (user_id, type, payload, status, channel, dedupe_key)
    values (v_order.buyer_id, 'ticket_purchase_succeeded',
            jsonb_build_object('orderId', v_order.id, 'orgId', v_order.org_id,
                               'amountCents', v_order.total_cents, 'currency', v_order.currency,
                               'ticketCount', v_issued),
            'pending', 'in_app', 'ticket_purchase_succeeded:' || v_order.id::text)
    on conflict do nothing;
  end if;

  -- The one effect that cannot be transactional: sending the tickets.
  insert into public.payment_outbox as ob (order_id, payment_id, topic, payload)
  values (v_order.id, v_payment.id, 'ticket_delivery', jsonb_build_object('orderId', v_order.id))
...
revoke execute on function public.fn_complete_order_payment(uuid, text, text, integer, text, jsonb) from public, anon, authenticated;
grant execute on function public.fn_complete_order_payment(uuid, text, text, integer, text, jsonb) to service_role;;
