-- SECURITY DEFINER RPC for initiating a ticket transfer by recipient email.
-- Looks up the recipient in auth.users, validates the order item is transferable,
-- and creates the transfer record in a single atomic operation.
