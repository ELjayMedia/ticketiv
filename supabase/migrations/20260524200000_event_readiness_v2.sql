-- Extend admin_event_readiness with the publish-gate checks added in TICK-37
-- so the super-admin dashboard surfaces the same readiness state organizers
-- see in the wizard publish step:
--   has_online_sales_channel  - at least one sellable ticket type has an
--                               online channel row.
--   has_refund_or_support     - refund_policy is non-empty jsonb or a
--                               confirmation_message is set.
--   has_active_staff          - at least one active event_staff row.
--   has_live_stats_row        - the event_live_stats counter row exists.
--
-- The view stays backward compatible: existing keys (`has_organization`,
-- `has_venue`, ...) keep their meaning; new keys appear in the `checks` jsonb.
