-- TICK-246: buyer-facing payment method management.
--
-- SECURITY DEFINER RPCs so the saved-payment-methods surface can set a default
-- and soft-remove a method without raw client-side updates on payment_methods.
-- Both functions authorize via auth.uid() and only ever touch the caller's own
-- rows.

begin;

-- Set the caller's default payment method (clears the flag on their others).
...
-- Soft-remove a payment method. If it was the default, promote the most recent
