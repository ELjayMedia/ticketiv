...
-- A security-invoker view gives admin/finance surfaces a stable, sortable queue
-- without opening direct writes to the base table.
...
begin
  -- 1. Paid order without a succeeded payment.
  ...
  select
    'paid_order_without_succeeded_payment',
    o.id::text,
    o.org_id,
    o.id,
    'critical',
  ...
  -- 2. Succeeded payment whose order did not reach paid.
  insert into public.finance_reconciliation_issues as fri (
    detector_key, entity_key, org_id, order_id, payment_id, severity, title, details,
    first_detected_at, last_detected_at, updated_at
  )
  select
    'succeeded_payment_without_paid_order',
    p.id::text,
    o.org_id,
    o.id,
    p.id,
    ...
    jsonb_build_object(
      ...
      'order_status', o.status,
      'provider', p.provider,
      'provider_reference', p.ext_payment_id,
  ...
  v_detected := v_detected + v_open;

  -- 3. Succeeded payment amount/currency differs from the order contract.
  insert into public.finance_reconciliation_issues as fri (
    detector_key, entity_key, org_id, order_id, payment_id, severity, title, details,
    first_detected_at, last_detected_at, updated_at
  )
  ...
  where p.status = 'succeeded'
    and (
      p.amount_cents <> o.total_cents
      or upper(coalesce(p.currency, '')) <> upper(coalesce(o.currency, ''))
    )
  on conflict (detector_key, entity_key) do update
  set org_id = excluded.org_id,
      order_id = excluded.order_id,
  ...
  -- 4. A provider reference must identify one payment only. Keep the queue
  -- platform-scoped because a collision can cross organizations.
  insert into public.finance_reconciliation_issues as fri (
  ...
  select
    ...
    d.provider || ':' || d.ext_payment_id,
    'critical',
    'Provider payment reference is duplicated',
    jsonb_build_object(
      'provider', d.provider,
      'provider_reference', d.ext_payment_id,
      'payment_ids', d.payment_ids,
  ...
  -- 5. Payment-composition ledger rows must always be linked to the payment
  -- that generated them. Null payment_id rows are creation-time pollution.
  insert into public.finance_reconciliation_issues as fri (
    detector_key, entity_key, org_id, order_id, severity, title, details,
    first_detected_at, last_detected_at, updated_at
  )
  select
    'payment_ledger_without_payment',
    le.id::text,
    le.org_id,
    le.order_id,
    'critical',
  ...
  -- 6. A succeeded payment attempt must link to the succeeded payment for the
  -- same order/provider/reference. This catches torn completion bookkeeping.
  insert into public.finance_reconciliation_issues as fri (
    detector_key, entity_key, org_id, order_id, payment_id, severity, title, details,
    first_detected_at, last_detected_at, updated_at
  )
  select
    'payment_attempt_link_mismatch',
    pa.id::text,
    o.org_id,
    pa.order_id,
    pa.payment_id,
    ...
    jsonb_build_object(
      ...
      'attempt_provider', pa.provider,
      'attempt_reference', pa.ext_ref,
      'attempt_payment_id', pa.payment_id,
      'linked_payment_status', p.status,
      'linked_payment_provider', p.provider,
  ...
  where pa.status = 'succeeded'
    and (
      pa.payment_id is null
      or p.id is null
      or p.status <> 'succeeded'
      or p.order_id is distinct from pa.order_id
      or p.provider is distinct from pa.provider
  ...
  -- Any managed issue not seen in this refresh is no longer an active
  -- discrepancy. A later recurrence reopens the same stable issue row.
  update public.finance_reconciliation_issues
  set status = 'resolved',
      resolved_at = v_started,
      updated_at = v_started
  ...
  return jsonb_build_object(
    'refreshed_at', v_started,
    'detected_this_run', v_detected,
    'auto_resolved_this_run', v_resolved,
    'open_issues', v_open,
    'critical_open_issues', v_critical
  );
end;
$function$;

revoke execute on function public.fn_refresh_finance_reconciliation_issues()
  from public, anon, authenticated;
grant execute on function public.fn_refresh_finance_reconciliation_issues() to service_role;

create or replace function public.fn_update_finance_reconciliation_issue(
  p_issue_id uuid,
  p_new_status text
)
...
revoke execute on function public.fn_update_finance_reconciliation_issue(uuid, text)
  from public, anon;
grant execute on function public.fn_update_finance_reconciliation_issue(uuid, text)
