-- TICK-33 follow-up: exclude non-on_sale tickets from price aggregation in
-- v_public_event_cards. The companion view v_events_public already filters
-- on sales_status (migration 20260524100916), but v_public_event_cards was
-- missed. Anon viewers were protected by the anon RLS policy on
-- ticket_types, but authenticated viewers whose policy allows reading the
-- whole ticket catalogue still see hidden comps drag min_price_cents to 0
-- on the homepage cards. Mirror the v_events_public filter here.
