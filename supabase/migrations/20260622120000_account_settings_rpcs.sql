-- TICK-207: unified account settings page.
--
-- Adds buyer-facing SECURITY DEFINER RPCs so the account settings surface can
-- persist profile fields and notification preferences without raw client-side
-- updates on protected tables. Both functions authorize via auth.uid() and only
-- ever touch the calling user's own rows.

begin;

-- Profile: update display name, name, surname and phone for the current user.
...
-- Notifications: update per-channel opt-in flags for the current user.
-- Upserts so the row exists even for users who never had a preferences row.
