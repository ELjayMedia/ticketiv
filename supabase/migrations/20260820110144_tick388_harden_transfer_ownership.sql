-- TICK-388: canonical per-ticket ownership + server-authoritative transfer lifecycle.
...
-- Preserve scanner hardening. The trigger permits only:
--   1) scanner check-in,
--   2) canonical owner initialization from orders.buyer_id,
--   3) a strict current_owner_id transfer mutation.
-- Authenticated clients have no UPDATE grant on order_items.
...
-- orders.buyer_id remains immutable purchase history; current_owner_id is the
-- effective holder for every individual ticket.
...
begin
  ...
  if exists (
    ...
    raise exception 'transfer_recipient_unavailable';
  end if;
...
revoke all on function public.fn_request_transfer_to_user_unchecked(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.fn_request_transfer_to_user_unchecked(uuid, uuid)
  to service_role;

create or replace function public.fn_request_transfer_to_user(
  p_order_item_id uuid,
  p_recipient_user_id uuid
)
returns jsonb
...
begin
  ...
  if not public.fn_rate_limit('transfer:' || auth.uid()::text, 20, 3600) then
    raise exception 'rate_limited: too many transfer requests, please try again later'
      using errcode = 'P0001';
  end if;
  return public.fn_request_transfer_to_user_unchecked(
    p_order_item_id,
    p_recipient_user_id
  );
end;
...
revoke all on function public.fn_request_transfer_to_user(uuid, uuid)
  from public, anon;
grant execute on function public.fn_request_transfer_to_user(uuid, uuid)
  to authenticated, service_role;

-- Preserve the existing json return type for backward compatibility.
create or replace function public.fn_request_transfer_by_email_unchecked(
  p_order_item_id uuid,
...
revoke all on function public.fn_request_transfer_by_email_unchecked(uuid, text)
  from public, anon, authenticated;
grant execute on function public.fn_request_transfer_by_email_unchecked(uuid, text)
  to service_role;

create or replace function public.fn_request_transfer_by_email(
  p_order_item_id uuid,
  p_recipient_email text
)
returns json
...
begin
  ...
  return public.fn_request_transfer_by_email_unchecked(
    p_order_item_id,
    p_recipient_email
  );
end;
...
revoke all on function public.fn_request_transfer_by_email(uuid, text)
  from public, anon;
grant execute on function public.fn_request_transfer_by_email(uuid, text)
  to authenticated, service_role;

create or replace function public.fn_request_transfer_by_phone(
  p_order_item_id uuid,
...
revoke all on function public.fn_request_transfer_by_phone(uuid, text)
  from public, anon;
grant execute on function public.fn_request_transfer_by_phone(uuid, text)
  to authenticated, service_role;
...
revoke all on function public.fn_complete_transfer_unchecked(uuid)
  from public, anon, authenticated;
grant execute on function public.fn_complete_transfer_unchecked(uuid)
  to service_role;
...
grant execute on function public.fn_complete_transfer(uuid)
  to authenticated, service_role;

create or replace function public.fn_decline_transfer(
  p_transfer_id uuid,
...
begin
  ...
  if not found then raise exception 'transfer_not_found'; end if;
  if v_transfer.to_user_id is distinct from auth.uid() then
    raise exception 'transfer_unauthorized';
  end if;
  if v_transfer.status not in (
    'pending'::public.transfer_status,
  ...
  set status = 'declined'::public.transfer_status,
      metadata = coalesce(metadata, '{}'::jsonb)
        || case
             when nullif(trim(p_reason), '') is null then '{}'::jsonb
  ...
  insert into public.notifications (
    ...
    jsonb_build_object('transfer_id', v_transfer.id, 'href', '/transfers'),
    'pending',
    'in_app',
    'ticket-transfer-declined:' || v_transfer.id::text
  ) on conflict do nothing;
...
grant execute on function public.fn_decline_transfer(uuid, text)
  to authenticated, service_role;
...
begin
  ...
  if not found then raise exception 'transfer_not_found'; end if;
  if v_transfer.from_user_id is distinct from auth.uid() then
    raise exception 'transfer_unauthorized';
  end if;
  if v_transfer.status not in (
    'pending'::public.transfer_status,
  ...
  update public.transfers
  set status = 'cancelled'::public.transfer_status,
      updated_at = now()
  where id = p_transfer_id;
...
grant execute on function public.fn_cancel_transfer(uuid)
  to authenticated, service_role;

-- Transfers remain readable through RLS, but state changes are RPC-only.
