-- SECURITY DEFINER RPC for toggling a user's event favourite.
-- Replaces raw client-side insert/delete so RLS policy is enforced
-- server-side and the caller cannot target another user's rows.
