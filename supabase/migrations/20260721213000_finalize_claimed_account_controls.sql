...
-- Platform-admin checks must fail closed for anonymous identities.
...
-- Remove direct anonymous table mutation surface.
...
-- Restrictive claimed-account policies compose with existing role/ownership
-- policies and prevent anonymous Supabase users from mutating protected rows.
drop policy if exists claimed_refunds_mutation on public.refunds;
create policy claimed_refunds_mutation on public.refunds
...
create policy claimed_refund_items_mutation on public.refund_items
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_payout_accounts_mutation on public.payout_accounts;
create policy claimed_payout_accounts_mutation on public.payout_accounts
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_payouts_mutation on public.payouts;
create policy claimed_payouts_mutation on public.payouts
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_devices_access on public.devices;
create policy claimed_devices_access on public.devices
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_device_sessions_access on public.device_sessions;
create policy claimed_device_sessions_access on public.device_sessions
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_admin_users_access on public.admin_users;
create policy claimed_admin_users_access on public.admin_users
as restrictive for all to authenticated
using (app.is_claimed_account())
with check (app.is_claimed_account());

drop policy if exists claimed_org_members_mutation on public.org_members;
create policy claimed_org_members_mutation on public.org_members
as restrictive for insert to authenticated
with check (app.is_claimed_account());

drop policy if exists claimed_org_members_update on public.org_members;
create policy claimed_org_members_update on public.org_members
as restrictive for update to authenticated
using (app.is_claimed_account()) with check (app.is_claimed_account());

drop policy if exists claimed_org_members_delete on public.org_members;
create policy claimed_org_members_delete on public.org_members
as restrictive for delete to authenticated
using (app.is_claimed_account());

drop policy if exists claimed_profiles_update on public.profiles;
create policy claimed_profiles_update on public.profiles
as restrictive for update to authenticated
using (app.is_claimed_account()) with check (app.is_claimed_account());

drop policy if exists claimed_guestlist_mutation on public.guestlist_entries;
create policy claimed_guestlist_mutation on public.guestlist_entries
as restrictive for all to authenticated
using (app.is_claimed_account()) with check (app.is_claimed_account());

-- Replace overly broad refund creation policy. A claimed buyer may request a
-- refund only for their own payment; finance/admin users may request for their org.
...
-- Payout rows are created through fn_request_payout and transitioned only by
-- the guarded admin RPC below. Direct authenticated mutation is removed.
...
begin
  ...
  if p_new_status not in ('processing','paid','failed','cancelled') then
    raise exception 'invalid_payout_status' using errcode='22023';
  end if;
...
create or replace function public.fn_register_device(
  ...
  p_event_id uuid,
  p_label text,
  p_device_role public.device_role
)
