...
export default function CreateEventPage() {
  ...
  const from = searchParams.get("from") || "header"

  const [authState, setAuthState] = useState<"guest" | "attendee" | "organizer" | null>(null)

  useEffect(() => {
    // Check auth state from demo session
    const demoSession = getDemoSession()
    if (demoSession) {
      setAuthState(demoSession.role === "organizer" ? "organizer" : "attendee")
    } else {
      setAuthState("guest")
    }
  }, [])
      ...
      <div className="min-h-screen bg-gradient-to-br from-background to-muted/50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>You're all set as an organizer</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Let's get your first event live. You can add details, set ticket prices, and publish whenever you're ready.
            </p>
            <Button className="w-full" asChild>
              <Link href="/orgs/new-org-123/events/new">Start Creating Event</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
